    ...
    while True:
        ...
        with torch.inference_mode():
            ...
            for i in range(len(output_ids["attentions"])):
                ...
                save_attn_all.append(save_attn_i)
            ...
            save_attn_all = np.array(save_attn_all)
            save_dict = {"sequences": save_sequences, "scores": save_scores, "attentions": save_attn, "attentions_text": save_attn_text, "attentions_all": save_attn_all}
            if args.vis_path is not None:
        ...
        if args.vis_path is not None:
            ...
            attns_all = save_dict['attentions_all']
            ...
            if args.plot_token_head:
                # visualize attention in each head
                # for i in range(len(tokens)):
                for i in [33]:
                    token = tokens[i]
                    attn_all = attns_all[i]
                    attn_all = (attn_all / args.attn_temp).astype(np.float32)
                    attn_all = attn_all.reshape(attn_all.shape[0], attn_all.shape[1], 24, 24)
                    print(attn_all.shape)
                    n_layers = attn_all.shape[0]
                    n_heads = attn_all.shape[1]
                    fig, axs = plt.subplots(n_layers, n_heads, figsize=(n_heads, n_layers))
                    for l in range(n_layers):
                        for h in range(n_heads):
                            ax = axs[l, h]
                            ax.imshow(attn_all[l, h], cmap='Reds', interpolation='nearest', vmin=0.0, vmax=1.0)
                            ax.set_xticks([])
                            ax.set_yticks([])
                            if l == 0:
                            ...
                            if h == 0:
                                ax.set_ylabel(f'Layer {l}', rotation=90)
                    plt.tight_layout()
                    plt.savefig(os.path.join(args.vis_path, file_name + f"_attn_all_{i}.png"))
                    plt.savefig(os.path.join(args.vis_path, file_name + f"_attn_all_{i}.pdf"))
                    plt.close()

            if args.plot_token_compact:
                # visualize attention maps
                plots_W = 10
                plots_H = (40 + plots_W - 1) // plots_W
                ...
                plt.savefig(os.path.join(args.vis_path, file_name + "_attn.pdf"))
                plt.close()

                # # visualize segmentation
                # plt.figure(figsize=(plots_W, plots_H))
                # for i in range(len(tokens)):
                #     token = tokens[i]
                #     attn = attns[i]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--attn-temp', type=float, default=0.0005)
    ...
    parser.add_argument('--plot-token-head', action='store_true')
