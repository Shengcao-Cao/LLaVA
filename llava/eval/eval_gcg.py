...
def evaluate_mask_miou(coco_gt, image_ids, pred_save_path):
    ...
    for image_id in tqdm(image_ids):
        ...
        gt_masks = [mask_utils.decode(ann['segmentation']) for ann in gt_anns if 'segmentation' in ann]
        ...
        pred_masks = [mask_utils.decode(ann['segmentation']) for ann in pred_anns if 'segmentation' in ann]
...
def find_best_matches(gt_anns, gt_labels, dt_anns, dt_labels, iou_threshold, text_sim_threshold, vectorizer=None):
    ...
    pred_masks = [mask_utils.decode(ann['segmentation']) for ann in dt_anns]
    gt_masks = [mask_utils.decode(ann['segmentation']) for ann in gt_anns]
...
def main():
    ...
    split = args.split
    assert split == "val" or split == "test" or split == "val_s100"  # GCG Evaluation has only val and test splits
    ...
    for image_id in all_images_ids:
        ...
        if not os.path.exists(prediction_path):
            caption_pred_dict[image_id] = {'caption': '', 'labels': []}
            continue
    ...
    coco_eval.evaluate()        # changed pycocoevalcap code to skip SPICE evaluation
